...
class DDosDetector:
    ...
    async def initialize_detection(self):
        ...
        try:
            ...
            async with self.session.put(
                f'{self.config.SFLOW_RT}/group/lf/json',
                json=self.config.GROUPS
            ) as resp:
    ...
    async def _configure_attack_detection(self, signature: AttackSignature):
        ...
        try:
            async with self.session.put(
                f'{self.config.SFLOW_RT}/flow/{signature.metric_name}/json',
                json=flows
            ) as resp:
            ...
            try:
                ...
                for flow_data, metadata in expired:
                    async with self.session.delete(
                        ...
                        logger.info(f"Unblocked {metadata.get('ip', 'unknown')}: {result.get('status')}")
    ...
    async def _process_event(self, event: dict):
        ...
        signature = next(
            (sig for sig in self.attack_signatures.values() if sig.metric_name == metric_name),
            None
        )
        ...
        try:
            ...
            if (metric.get('metricValue', 0) > signature.threshold and 
                metric.get('topKeys')):
    ...
    async def _block_source(self, key: str, signature: AttackSignature):
        ...
        try:
            async with self.session.post(
                ...
                result = await resp.json()
                logger.warning(f"🚨 BLOCKED {src_ip} -> {dst_ip} ({signature.name}): {result.get('status')}")
